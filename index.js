...
async function openPage(cookiesString, userAgentString) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (cookiesString) {
        const cookies = cookiesString.split(';').map(pair => {
          const [name, value] = pair.split('=').map(component => component.trim());
...
async function executeInParallel() {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const cookiesString = cookiesArray[i] || '';
    promises.push(openPage(cookiesString, userAgent));
  }
