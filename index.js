...
// دالة لتحويل نص الكوكيز إلى كائنات كوكيز
...
async function openBrowser(url, duration, userAgent, cookies = null) {
...
if (process.argv[2] === 'child') {
    ...
    const cookieString = process.argv[6];
    const cookies = cookieString ? parseCookies(cookieString) : null;
    openBrowser(url, duration, userAgent, cookies).catch(err => {
        console.error(err);
        process.exit(1);
    });
} else {
    ...
    function openBrowsers() {
        ...
        for (let i = 0; i < browserCount; i++) {
            ...
            const cookieString = cookiesArray[i] || ''; // الحصول على نص الكوكيز إذا كان متاحاً
            const child = spawn('node', [__filename, 'child', url, duration, userAgent, cookieString], { stdio: 'inherit' });
